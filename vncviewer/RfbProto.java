...
class RfbProto {
  ...
  void readVersionMsg() throws Exception {
	...
	|| (b[10] < '0') || (b[10] > '9') || (b[11] != '\n'))
    {
      throw new Exception("Server sent unrecognized handshake");
    }
    ...
    if (serverMajor != 3 || serverMinor != 8) {
      throw new Exception("RFB server uses unsupported protocol version");
    }
  }
  ...
  int negotiateSecurity() throws Exception {
    ...
    for (int i = 0; i < nSecTypes; i++) {
      if (secTypes[i] == SecTypeNone || secTypes[i] == SecTypeAES) {
	secType = secTypes[i];
	break;
      }
    }
  ...
  //
  // Perform AES cookie authentication.
  //
  ...
  void initCapabilities() {
    ...
    authCaps.add(AuthNone, StandardVendor, SigAuthNone,
		 "No authentication");
    authCaps.add(AuthAES, StandardVendor, SigAuthAES,
		 "AES cookie authentication");
  ...
  int negotiateAuthenticationTight() throws Exception {
    ...
    for (int i = 0; i < authCaps.numEnabled(); i++) {
      ...
      if (authType == AuthNone || authType == AuthAES) {
	writeInt(authType);
	return authType;
      }
    }
